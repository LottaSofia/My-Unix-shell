/* Made by: Lotta Sofia Tuominen, December 2018 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[]) {
	...
	FILE *tiedosto;
	char muisti[256];
	if (argc > 1) { // If the user gives a file name as a command-line argument
		...
		do {
			...
			if (strcmp(rivi,"\n") == 0) { // If user presses enter the program goes back to the beginning of the loop
				continue;
			} else {
				tila = wish_komennonSelvitys(syotteet);
			}
...
int wish_komennonSelvitys(char **a) {
	char *b_i[] = {"exit","path","cd"}; // build in list, that can be compared to the command
	if (a[0] == NULL) {
		return 1;
	}
	...
	return wish_kaynnistys(a); // If the command is build in
}
...
int wish_kaynnistys(char **args) {
	...
	int status;
	char polku[20] = "/bin/";
	strcat(polku, args[0]); // The command is added to the path (for example path = /bin/ls)

	int i = 0;
	while (args[i] != NULL) { // Here we see if there is rederict sign ">"
		if (strcmp(args[i],">") == 0) {
			args[i] = NULL;
			break;
		}
	...
	pid = fork(); // Source: [https://brennan.io/2015/01/16/write-a-shell-in-c/]
	if (pid == 0) {
...
char **wish_kaskyjen_halkaisu(char *rivi) { // Create a list where the "words" of the command are separated
	size_t palaSize = 64;
	...
	while (pala != NULL) {
		...
		if (sizeof(palat) <= palaSize) {
			v_pala = palat;
			palaSize += 64;
			palat = realloc(palat,palaSize * sizeof(char)); // Incase it's needed, realloc more memory
		}
